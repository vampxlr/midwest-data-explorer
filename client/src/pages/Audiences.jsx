  ...
  return (
    <div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:12}}>
        ...
        {/* Force re-fetch — purges stored contacts first so count-unchanged skip is bypassed */}
        <button disabled={fetching}
          onClick={()=>{
          ...
          {!previewing && preview && (
            <div style={{background:'#0e1018',border:'1px solid #252838',borderRadius:10,padding:'14px',marginBottom:16}}>
              <div style={{display:'flex',gap:20,marginBottom:12,flexWrap:'wrap'}}>
                <div><div style={{fontSize:10,color:'#64748b',textTransform:'uppercase',letterSpacing:'0.5px'}}>Registrations</div>
                  <div style={{fontSize:28,fontWeight:800,color:'#94a3b8'}}>{preview.total}</div></div>
                <div><div style={{fontSize:10,color:'#64748b',textTransform:'uppercase',letterSpacing:'0.5px'}}>Unique Emails (all players)</div>
                  <div style={{fontSize:28,fontWeight:800,color:'#60a5fa'}}>{preview.uniqueEmails ?? preview.withEmail}</div>
                  <div style={{fontSize:9,color:'#334155'}}>1 row per email in CSV</div></div>
              </div>
          ...
          {!previewing && preview && (
            <div style={{background:'#0e1018',border:'1px solid #252838',borderRadius:10,padding:'14px 16px',marginBottom:16}}>
              <div style={{display:'flex',gap:20,flexWrap:'wrap',marginBottom:preview.gender?.length>0?12:0}}>
                <div>
                  <div style={{fontSize:10,color:'#64748b',textTransform:'uppercase',letterSpacing:'0.5px'}}>Registrations</div>
                  <div style={{fontSize:32,fontWeight:800,color:'#94a3b8',letterSpacing:'-1px'}}>{preview.total}</div>
                  <div style={{fontSize:11,color:'#475569'}}>team sign-ups</div>
                </div>
                <div>
                  <div style={{fontSize:10,color:'#64748b',textTransform:'uppercase',letterSpacing:'0.5px'}}>CSV Rows (all player emails)</div>
                  <div style={{fontSize:32,fontWeight:800,color:'#22c55e',letterSpacing:'-1px'}}>{preview.uniqueEmails ?? preview.withEmail}</div>
                  <div style={{fontSize:11,color:'#475569'}}>one row per unique email</div>
                </div>
  ...
  function LeagueChips({ items, onRemove }) {
    return (
      <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginTop:8 }}>
        {items.map(l => (
          <div key={l.id} style={{ display:'flex', alignItems:'center', gap:5, background:'#1e2235',
            border:'1px solid #2a2d3e', borderRadius:20, padding:'4px 10px 4px 12px', fontSize:11 }}>
        ...
        {items.length === 0 && <p style={{ color:'#334155', fontSize:11, margin:0 }}>No leagues selected yet.</p>}
      </div>
    );
  }
  ...
  return (
    <div className="card" style={{ marginTop: 24 }}>
      ...
      <p style={{ color:'#475569', fontSize:12, margin:'0 0 16px', lineHeight:1.6 }}>
        Find every individual player from past leagues who did <strong style={{color:'#94a3b8'}}>not</strong> sign up
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        ...
        {/* SOURCE */}
        <div style={{ background:'#0e1018', borderRadius:10, padding:'12px 14px', border:'1px solid #1e2235' }}>
            ...
            {[{id:'year',label:'By Year'},{id:'custom',label:'Pick Leagues'}].map(m => (
              <button key={m.id} onClick={() => { setSourceMode(m.id); setData(null); }}
                style={{ padding:'5px 14px', borderRadius:7, fontSize:11, fontWeight:700, border:'none', cursor:'pointer',
                  background: sourceMode===m.id ? '#1e3a5f':'#1e2235',
                  color:      sourceMode===m.id ? '#60a5fa':'#64748b',
          ...
          {sourceMode === 'year' ? (
            <>
              <label style={{ display:'block', fontSize:10, color:'#64748b', marginBottom:5, fontWeight:600, textTransform:'uppercase' }}>Source Year</label>
                ...
                {years.map(y => (
                  <button key={y} onClick={() => { setSourceYear(y); setData(null); }}
                    style={{ padding:'6px 14px', borderRadius:7, fontSize:12, fontWeight:700, border:'none', cursor:'pointer',
                      background: sourceYear===y ? '#2563eb':'#1e2235', color: sourceYear===y ? '#fff':'#64748b' }}>
                    {y}
                  </button>
                ))}
              </div>
            ...
            <>
              <label style={{ display:'block', fontSize:10, color:'#64748b', marginBottom:5, fontWeight:600, textTransform:'uppercase' }}>Add Source Leagues</label>
              <SearchableSelect
                value=""
                onChange={id => {
                  const r = recentRegs.find(x => String(x.id) === String(id));
        ...
        <div style={{ background:'#0e1018', borderRadius:10, padding:'12px 14px', border:'1px solid #1e2235' }}>
          ...
          <div style={{ display:'flex', gap:6, marginBottom:10 }}>
            {[{id:'year',label:'By Year'},{id:'league',label:'Specific League'}].map(m => (
              <button key={m.id} onClick={() => { setExcludeMode(m.id); setData(null); }}
                style={{ padding:'5px 14px', borderRadius:7, fontSize:11, fontWeight:700, border:'none', cursor:'pointer',
                  background: excludeMode===m.id ? '#3b1f5e':'#1e2235',
                  color:      excludeMode===m.id ? '#c084fc':'#64748b',
          ...
          {excludeMode === 'year' ? (
            <>
              <label style={{ display:'block', fontSize:10, color:'#64748b', marginBottom:5, fontWeight:600, textTransform:'uppercase' }}>Exclude Year</label>
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {years.map(y => (
                  <button key={y} onClick={() => { setExcludeYear(y); setData(null); }}
                    style={{ padding:'6px 14px', borderRadius:7, fontSize:12, fontWeight:700, border:'none', cursor:'pointer',
                      background: excludeYear===y ? '#7c3aed':'#1e2235', color: excludeYear===y ? '#fff':'#64748b' }}>
                    {y}
                  </button>
                ))}
              </div>
            ...
            <>
              <label style={{ display:'block', fontSize:10, color:'#64748b', marginBottom:5, fontWeight:600, textTransform:'uppercase' }}>Add Exclude Leagues</label>
              <SearchableSelect
                value=""
                onChange={id => {
                  const r = recentRegs.find(x => String(x.id) === String(id));
      ...
      <button onClick={run} disabled={!canRun || loading}
        style={{ padding:'10px 28px', borderRadius:8, fontSize:13, fontWeight:700, border:'none',
          ...
          background: canRun ? '#2563eb':'#1e2235', color: canRun ? '#fff':'#64748b',
      ...
      {data && (
        <>
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginBottom:12 }}>
            {[
              { label:'Total unique individuals', val: data.totalIndividuals, color:'#94a3b8', sub:'from source leagues, deduplicated' },
          ...
          {gyBreakdown.length > 0 && (
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:10, color:'#64748b', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:6 }}>
                Grad Year Breakdown (lapsed)
              </div>
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {gyBreakdown.map(({ year, count }) => (
                  <div key={year} style={{ background:'#1e2235', borderRadius:6, padding:'4px 10px', fontSize:11, cursor:'pointer',
                    border: (gyFrom===year || gyTo===year) ? '1px solid #60a5fa':'1px solid transparent' }}
                    title={`${count} lapsed from grad year ${year}`}
                    onClick={() => {
                      if (!gyFrom || gyFrom > year) setGyFrom(year);
                      if (!gyTo   || gyTo   < year) setGyTo(year);
                    }}>
                    <span style={{ color:'#94a3b8', fontWeight:700 }}>{year}</span>
                    <span style={{ color:'#475569', marginLeft:5 }}>{count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {data.lapsedCount > 0 && (
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12, flexWrap:'wrap' }}>
              <span style={{ fontSize:10, color:'#64748b', fontWeight:700, textTransform:'uppercase' }}>Filter by grad year:</span>
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                <input
                  type="number" min="2020" max="2040" placeholder="From"
                  value={gyFrom} onChange={e => setGyFrom(e.target.value)}
                  style={{ width:72, background:'#1e2235', border:'1px solid #252838', color:'#f1f5f9', borderRadius:6, padding:'5px 8px', fontSize:12, outline:'none' }}
                />
                ...
                {(gyFrom || gyTo) && (
                  <button onClick={() => { setGyFrom(''); setGyTo(''); }}
                    style={{ fontSize:11, background:'none', border:'none', color:'#475569', cursor:'pointer', padding:'2px 6px' }}>
                    ✕ clear
                  </button>
                )}
              </div>
              ...
              {showTable && (
                <div style={{ overflowX:'auto', maxHeight:420, overflowY:'auto' }}>
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
                    <thead>
                      <tr style={{ borderBottom:'1px solid #1e2235', position:'sticky', top:0, background:'#0e1018' }}>
                        {['Name','Email','Phone(s)','Grad Year(s)','Source League(s)'].map(h => (
                          <th key={h} style={{ textAlign:'left', padding:'6px 10px', color:'#64748b', fontSize:10, textTransform:'uppercase', whiteSpace:'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredLapsed.map((p, i) => (
                        <tr key={i} style={{ borderBottom:'1px solid #12141c', background: i%2===0?'transparent':'#0a0c12' }}>
                          <td style={{ padding:'5px 10px', color:'#e2e8f0', whiteSpace:'nowrap' }}>{p.name || <span style={{color:'#334155'}}>—</span>}</td>
                          <td style={{ padding:'5px 10px', maxWidth:220 }}>
                            <a href={`mailto:${p.email}`} style={{ color:'#60a5fa', textDecoration:'none', fontSize:11 }}>{p.email}</a>
                          </td>
                          ...
                          <td style={{ padding:'5px 10px' }}>
                            {(p.gradYears||[]).length > 0
                              ? (p.gradYears||[]).map((gy, gi) => (
                                  <span key={gi} style={{ display:'inline-block', background:'#1e2235', borderRadius:4,
                          ...
                          <td style={{ padding:'5px 10px', color:'#64748b', fontSize:10 }}>
